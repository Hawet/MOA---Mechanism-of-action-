...
def build_model(train_features):
    ...
    model.compile(optimizer=tfa.optimizers.AdamW(lr = 1e-3, weight_decay = 1e-5, clipvalue = 756),
                  loss=tf.keras.losses.BinaryCrossentropy(label_smoothing=1e-15),
                  )
    return model
...
def k_fold_model_estimator(SPLITS,features,targets):
    ...
    for i in range(SPLITS):
        ...
        save_weights_only=False, verbose=1),epochs=15,batch_size=128)
        history.append(hist)
...
train_features = pd.read_csv("train_features.csv", encoding="utf-8-sig")
test_features = pd.read_csv("test_features.csv", encoding="utf-8-sig")
...
model = k_fold_model_estimator(1,train_features,train_targets_scored)
